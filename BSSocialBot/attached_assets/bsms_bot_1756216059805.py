...
def get_semester_start_date(semester_code):
    ...
    try:
        ...
        return datetime.date(year, 8, 30) if "FA" in semester_code else datetime.date(year, 1, 30)
    except ValueError:
        return None
...
def semesters_between(start_code, end_code):
    ...
    return (e_year - s_year) * 2 + (semesters.index(e_term) - semesters.index(s_term))
...
def validate_course(course, existing_courses):
    ...
    if not valid_pattern.match(course):
        warnings.append(f"❌ {course} is not a valid course code format (e.g., CMSC420, STAT410).")
        return warnings

    # Disallowed courses
    if course.startswith("CMSC498") or course.startswith("CMSC499"):
        warnings.append(f"⚠️ {course} is a CMSC498/499 course and NOT eligible for the combined BS/MS form.")
    if course in ["CMSC320", "CMSC335"] or course.startswith("CMSC3"):
        warnings.append(f"⚠️ {course} is a 300-level course and NOT eligible for the combined BS/MS form.")
    if course == "CMSC412":
        warnings.append(f"⚠️ {course} is a 4-credit course, and you can only double count two courses totaling 7 credits, since the max double-count cannot exceed 9 credits.")
    if course.startswith("CMSC6") or course.startswith("CMSC7"):
        warnings.append(f"⚠️ {course} is a graduate-level course. You must get departmental approval to take this course for undergraduate credit only.")
    if course.startswith(("AMSC", "STAT", "MATH")) and any(c.startswith(("AMSC", "STAT", "MATH")) for c in existing_courses):
        warnings.append(f"⚠️ {course} is an AMSC/STAT/MATH course. Only one such course can be included in the 9-credit double-count limit.")
...
def handle_step(session_data, message):
    ...
    if step == 0:
        ...
        if message.lower() != "yes":
            return "❌ Only CS majors are eligible for the Combined BS/MS Program.", {"step": None, "responses": {}}
        responses["is_cs_major"] = message
        session_data["step"] = 1
        return (
            "Q1. What is your current level of CS courses?\n\n"
            "1. CMSC330 and CMSC351 completed\n"
            "2. Currently in CMSC330 and CMSC351\n"
            "3. Currently in CMSC216 and CMSC250\n"
            "4. Taking CMSC131 or CMSC132",
            session_data
        )
    ...
    # Step 2: Graduation semester
        ...
        current_sem = "FA" + str(today.year) if today.month >= 6 else "SP" + str(today.year)
        if semesters_between(current_sem, grad_sem) < 2:
            return "⚠️ You are too close to graduation to use this tool. Meet with your advisor ASAP.", None
    ...
    # Step 3: GPA
        ...
        return msg + "\n\nQ4: Will you finish Gen Eds by 2 semesters before graduation? (yes/no)", session_data

    # Step 4: Gen Ed completion
    elif step == 4:
        if message.lower() not in ["yes", "no"]:
            return "❌ Please answer 'yes' or 'no'. Will you finish Gen Eds by 2 semesters before graduation?", session_data
    ...
    # Step 5: Total credits
        ...
        if credits < 90:
            ...
            return ("⚠️ You must have at least 90 credits by 2 semesters before graduation.\n\n"
        ...
        return (
            ...
            "Hit OK to coninue!", session_data
        )
    # Step 5 warning path
    elif step == "5_warn":
    ...
    # Step 6: Course input
        ...
        return (
            "Hit Enter after typing your first course:", 
            session_data
        )
        ...
        warnings = validate_course(course, session_data["responses"]["course_list"])
    ...
    # Step 9: Final checklist
        ...
        credit_check = responses.get("credit_check_sem")
        gpa = responses.get("gpa", 0.0)
        gen_ed_ok = responses.get("gen_ed_complete") == "yes" or responses.get("gen_ed_credits_remaining", 0) <= 6
        ...
        checklist = []
        checklist.append("- ✅ GPA ≥ 3.5" if gpa >= 3.5 else "- ❌ GPA below 3.5")
        checklist.append("- ✅ 90+ credits by 2 semesters prior" if credit_ok else "- ❌ Less than 90 credits")
        checklist.append("- ✅ Gen Eds ≤6 credits remaining" if gen_ed_ok else "- ❌ Too many Gen Eds remaining")
        checklist.append("- ✅ Research topic and advisor secured" if research == "1" else "- ⚠️ Work on research/advisor")

        if courses:
            if course_warnings > 0:
                checklist.append(f"- ⚠️ {course_warnings} course warning(s). See an advisor to plan course selection properly.")
            else:
                checklist.append(f"- ✅ {len(courses)} courses selected and validated")
        ...
        if abroad and abroad_sem:
            ...
            if semesters_between(abroad_sem, grad_sem) <= 2:
                checklist.append("- ⚠️ Study Abroad close to graduation — transfer credits may not count")

        summary = [
            "📋 --- YOUR PERSONALIZED BS/MS PLAN ---",
            f"🎓 Graduation Semester: {grad_sem}",
            f"➡️ Start MS Program: {ms_start}",
            f"📝 Submit BS/MS form in: {credit_check}",
            f"📚 Take 3 courses for BS/MS in {credit_check}",
            "🗓 Grad School Deadlines: Fall - Dec prior | Spring - Sep prior",
            "",
            "✅ CHECKLIST"
        ] + checklist + [
            "",
            "📎 Form: https://go.umd.edu/bsms-form",
            "💬 Talk to a CS advisor for next steps.",
            "",
            "🎉 Done! Thanks for using the UMD CS BS/MS Planner.",
            "",
            "⚠️ Disclaimer: This program is a guideline, please consult with your assigned advisor for better guidance."
        ]
